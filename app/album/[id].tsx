 ...
 return (
   ...
   >
     {[1, 2, 3, 4, 5].map((s) => {
       ...
       const half = !filled && currentRating >= s - 0.5;
       return (
         <View key={s} style={{ width: STAR_WIDTH, height: STAR_WIDTH }}>
           <Text style={styles.starEmpty}>★</Text>
           {(filled || half) && (
             <View style={{
               position: 'absolute', top: 0, left: 0,
               overflow: 'hidden',
               width: filled ? STAR_WIDTH : STAR_WIDTH / 2
             }}>
...
export default function AlbumPage() {
 ...
 function makeUpdateRating(
   ...
   return (pageX: number) => {
     ...
     const { x } = layoutRef.current;
     const relX = pageX - x;
     const totalWidth = 5 * STAR_WIDTH + 4 * STAR_GAP;
     ...
     setRatingFn(newRating);
     AsyncStorage.getItem('reviews').then(existing => {
       const list = existing ? JSON.parse(existing) : [];
       const idx = list.findIndex((i: any) => i.albumId === getItemId());
       if (idx !== -1) {
         list[idx].rating = newRating;
       } else {
         list.unshift({
           albumId: getItemId(),
           albumName: getItemName(),
 ...
 const updateAlbumRating = useRef(
   makeUpdateRating(
     setRating,
     starsLayout,
     () => album?.id,
 ...
 const panResponder = useRef(
   PanResponder.create({
     ...
     onPanResponderGrant: (e) => updateAlbumRating.current(e.nativeEvent.pageX),
     onPanResponderMove: (e) => updateAlbumRating.current(e.nativeEvent.pageX),
   })
 ).current;

 const updateTrackRatingFn = useRef(
   makeUpdateRating(
     setTrackRating,
     trackStarsLayout,
     () => selectedTrack?.id,
 ...
 const trackPanResponder = useRef(
   PanResponder.create({
     onStartShouldSetPanResponder: () => true,
     onMoveShouldSetPanResponder: () => true,
     onPanResponderGrant: (e) => updateTrackRatingFn.current(e.nativeEvent.pageX),
     onPanResponderMove: (e) => updateTrackRatingFn.current(e.nativeEvent.pageX),
   })
 ).current;

 // გამოვიყენოთ rating state პირდაპირ render-ში
 useEffect(() => {
   updateAlbumRating.current = makeUpdateRating(
     setRating, starsLayout, () => album?.id, () => album?.name
   );
 }, [album]);

 useEffect(() => {
   updateTrackRatingFn.current = makeUpdateRating(
     setTrackRating, trackStarsLayout, () => selectedTrack?.id, () => selectedTrack?.name
   );
 }, [selectedTrack]);
   ...
   <ScrollView style={styles.container}>
     ...
     <Modal visible={menuVisible} transparent animationType="slide">
       ...
       <View style={styles.sheet}>
         ...
         <View style={styles.rateSection}>
           ...
           {renderStarRow(rating, starsLayout, panResponder.panHandlers)}
         </View>
     ...
     <Modal visible={trackMenuVisible} transparent animationType="slide">
       ...
       <View style={styles.sheet}>
         ...
         <View style={styles.rateSection}>
           <Text style={styles.rateLabel}>Rate</Text>
           {renderStarRow(trackRating, trackStarsLayout, trackPanResponder.panHandlers)}
         </View>
...
const styles = StyleSheet.create({
 ...
 starsRow: { flexDirection: 'row', gap: STAR_GAP },
 starEmpty: { fontSize: 36, color: '#333', position: 'absolute' },
 starFilled: { fontSize: 36, color: '#ffb6c1', position: 'absolute' },
